// test_yfp_ypfp_axisvalley_debug_candidates.C
//
...
// Usage:
//   hcana -l
//   .x test_yfp_ypfp_axisvalley_debug_candidates.C(1544,"axisvalley_debug1","-1","multifoil_test1",0,2)
//
// Main knobs:
...
void test_yfp_ypfp_axisvalley_debug_candidates(Int_t nrun=1544,
                              const char *tag="axisvalley_debug1",
                              ...
                              Double_t maxNeighborFrac=0.18)
{
  ...
  TString outPdf = Form("plots/test_yfp_ypfp_axisvalley_debug_candidates_run%d_%s.pdf", nrun, outTag.Data());

  TCanvas *c = new TCanvas("c_axisvalley_debug", "axis-valley candidate-by-candidate diagnostic", 1100, 850);
  TLatex tx;
  tx.SetNDC();
  ...
  cout << "  find more peaks: .x test_yfp_ypfp_one_island_axisvalley.C(" << nrun << ",\"" << outTag << "_morePeaks\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << ",0.035,0.30,0," << uSigmaScale << "," << minUHalf << "," << maxUHalf << "," << minPurity << "," << maxNeighborFrac << ")" << endl;
  cout << "  narrower transverse guard: .x test_yfp_ypfp_one_island_axisvalley.C(" << nrun << ",\"" << outTag << "_narrowU\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << "," << minPeakSepS << "," << smoothPassesS << ",1.25," << minUHalf << "," << maxUHalf << "," << minPurity << "," << maxNeighborFrac << ")" << endl;
