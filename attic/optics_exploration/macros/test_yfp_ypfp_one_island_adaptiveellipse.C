// test_yfp_ypfp_one_island_adaptiveellipse.C
//
// Hyper-focused adaptive-ellipse development macro for the Yfp/YpFp selection problem.
// Goal: in ONE delta slice, find ONE local high-density YpFp/YFp island
// and grow it adaptively while it maps cleanly onto one Ys guide line in the Xs/Ys plane.
//
...
void ScoreCandidate(LocalCandidate &cand,
  ...
  // The adaptive chooser below will usually select the largest acceptable ellipse.
...
bool PassesAdaptiveCriteria(const LocalCandidate &cand,
                            double minPurity,
                            double maxNeighborFrac,
                            int minSelected) {
  if (cand.nSelected < minSelected) return false;
  if (cand.purity < minPurity) return false;
  if (cand.neighborFrac > maxNeighborFrac) return false;
  return true;
}

void test_yfp_ypfp_one_island_adaptiveellipse(Int_t nrun=1544,
                              ...
                              Double_t nSigmaMajorBase=1.2,
                              Double_t nSigmaMinorBase=0.75,
                              Double_t scaleMin=0.8,
                              Double_t scaleMax=2.4,
                              Double_t scaleStep=0.2,
                              ...
                              Double_t maxNeighborFrac=0.18)
{
  ...
  vector<LocalCandidate> accepted;

  cout << "Adaptive ellipse scan:" << endl;
  cout << "  peak scale nSelected yscol purity neighborFrac quality accepted" << endl;
  ...
  for (size_t ip = 0; ip < peaks.size(); ip++) {
    for (double scale = scaleMin; scale <= scaleMax + 1e-9; scale += scaleStep) {
      LocalCandidate cand = BuildLocalEllipseCandidate(events, hSmooth,
                                                       ...
                                                       nSigmaMajorBase * scale,
                                                       nSigmaMinorBase * scale,
                                                       100);
      cand.index = (int)ip;
      cand.scaleFactor = scale;
      candCounter++;
      if (!cand.cut) continue;
      cand.cut->SetName(Form("adaptive_ellipse_peak%zu_scale_%03d", ip, (int)round(100.0*scale)));
      ScoreCandidate(cand, events, ysWindow, minSelected);
      bool ok = PassesAdaptiveCriteria(cand, minPurity, maxNeighborFrac, minSelected);
      candidates.push_back(cand);
      if (ok) accepted.push_back(cand);
  ...
  if (candidates.empty()) {
    cerr << "ERROR: no usable local candidate found. Try lowering minPeakFracOfMax or increasing seedHalfYpFp/seedHalfYFp." << endl;
    return;
  }

  // Prefer the largest clean candidate. If none pass the criteria, fall back to the highest quality candidate.
  LocalCandidate best;
  if (!accepted.empty()) {
    sort(accepted.begin(), accepted.end(), [](const LocalCandidate &a, const LocalCandidate &b) {
      if (a.yscol != b.yscol) return a.quality > b.quality;
      return a.nSelected > b.nSelected;
    });

    // Choose the accepted candidate with the largest event count, while still keeping quality sane.
    best = accepted[0];
    for (auto &c : accepted) {
      if (c.nSelected > best.nSelected && c.quality > 0.75 * accepted[0].quality) best = c;
    }
  } else {
    ...
    best = candidates[0];
    cout << "WARNING: no candidate passed adaptive purity/neighbor criteria; using highest quality fallback." << endl;
  }

  best.cut->SetName(Form("best_adaptive_candidate_yscol_%d_nfoil_%d_ndel_%d", best.yscol, foilIndex, ndel));
  ...
  cout << "  neighborScore  = " << best.neighborScore << endl;
  cout << "  neighborFrac   = " << best.neighborFrac << endl;
  cout << "  scaleFactor    = " << best.scaleFactor << endl;
  cout << "  knobs          = minPeakFracOfMax=" << minPeakFracOfMax
       << " seedHalfYpFp=" << seedHalfYpFp
       << " seedHalfYFp=" << seedHalfYFp
       << " nSigmaMajorBase=" << nSigmaMajorBase
       << " nSigmaMinorBase=" << nSigmaMinorBase
       << " scaleMin=" << scaleMin
       << " scaleMax=" << scaleMax
       << " scaleStep=" << scaleStep
       << " minPurity=" << minPurity
  ...
  TString outPdf = Form("plots/test_yfp_ypfp_one_island_adaptiveellipse_run%d_%s.pdf", nrun, outTag.Data());

  TCanvas *c = new TCanvas("c_test_one_island", "test one adaptive normalized local YpFp/YFp island", 1100, 850);
  TLatex tx;
  tx.SetNDC();
  ...
  gPad->SetLogz(1);
  hYAll->Draw("colz");
  for (auto &cscan : candidates) {
    if (!cscan.cut) continue;
    if (cscan.index != best.index) continue;
    cscan.cut->SetLineColor(kGray+1);
    cscan.cut->SetLineWidth(1);
    cscan.cut->Draw("L same");
  }
  ...
  lab->SetTextAlign(22);
  lab->Draw("same");
  tx.DrawLatex(0.12, 0.94, Form("All YpFp/YFp events; best adaptive candidate overlaid | purity=%.3f n=%d", best.purity, best.nSelected));
  c->Print(outPdf);
  gPad->SetLogz(0);
  ...
  hYsXsBest->Draw("colz");
  DrawYsGuideLines();
  tx.DrawLatex(0.12, 0.94, Form("Selected events only: assigned Ys column %d | purity=%.3f neighbor=%.3f n=%d score=%d", best.yscol, best.purity, best.neighborFrac, best.nSelected, best.bestScore));
  c->Print(outPdf);
  gPad->SetLogz(0);
  ...
  cout << "  stricter purity: .x test_yfp_ypfp_one_island_adaptiveellipse.C(" << nrun << ",\"" << outTag << "_strict\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << "," << localRelLevel << "," << seedHalfYpFp << "," << seedHalfYFp << "," << nSigmaMajorBase << "," << nSigmaMinorBase << "," << scaleMin << "," << scaleMax << "," << scaleStep << ",0.78," << maxNeighborFrac << ")" << endl;
  cout << "  allow larger clean growth: .x test_yfp_ypfp_one_island_adaptiveellipse.C(" << nrun << ",\"" << outTag << "_bigger\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << "," << localRelLevel << "," << seedHalfYpFp << "," << seedHalfYFp << "," << nSigmaMajorBase << "," << nSigmaMinorBase << "," << scaleMin << ",3.0," << scaleStep << "," << minPurity << "," << maxNeighborFrac << ")" << endl;
