// test_yfp_ypfp_axisvalley_coordinate_debug2.C
//
...
// Usage:
//   hcana -l
//   .x test_yfp_ypfp_axisvalley_coordinate_debug2.C(1544,"axiscoord_debug1","-1","multifoil_test1",0,2)
//
// Main knobs:
...
void DrawAxisCoordinateGrid(const AxisBasis &basis,
  ...
  for (auto &cnd : candidates) {
    DrawConstantSLine(basis, cnd.sLo, uMinDraw, uMaxDraw, kBlack, 3, 2);
    DrawConstantSLine(basis, cnd.sHi, uMinDraw, uMaxDraw, kBlack, 3, 2);
  }
...
void test_yfp_ypfp_axisvalley_coordinate_debug2(Int_t nrun=1544,
                              ...
                              Double_t maxNeighborFrac=0.18)
{
  ...
  TH2D *hSUAll = new TH2D("hSUAll",
                          Form("Run %d foil %d ndel %d: transformed principal-axis coordinates;s coordinate;u coordinate", nrun, foilIndex, ndel),
                          nBinsS, sMin, sMax,
                          180, uMinDraw, uMaxDraw);
  for (auto &e : events) hSUAll->Fill(e.s, e.u);
  ...
  TString outPdf = Form("plots/test_yfp_ypfp_axisvalley_coordinate_debug2_run%d_%s.pdf", nrun, outTag.Data());
  ...
  // Page 1: transformed coordinates. This is the cleanest way to debug
  // whether the principal-axis coordinate system is doing what we think.
  c->Clear();
  gPad->SetLogz(1);
  hSUAll->Draw("colz");

  double uRangeMin = uMinDraw;
  double uRangeMax = uMaxDraw;
  for (auto &cnd : candidates) {
    // Candidate rectangle in the actual s-u space used by the cut.
    double uLo = cnd.uCenter - cnd.uHalf;
    double uHi = cnd.uCenter + cnd.uHalf;

    TLine *l1 = new TLine(cnd.sLo, uLo, cnd.sHi, uLo);
    TLine *l2 = new TLine(cnd.sHi, uLo, cnd.sHi, uHi);
    TLine *l3 = new TLine(cnd.sHi, uHi, cnd.sLo, uHi);
    TLine *l4 = new TLine(cnd.sLo, uHi, cnd.sLo, uLo);
    int ccol = CandidateIsAccepted(cnd, minSelected, minPurity, maxNeighborFrac) ? kGreen+2 : kOrange+7;
    l1->SetLineColor(ccol); l2->SetLineColor(ccol); l3->SetLineColor(ccol); l4->SetLineColor(ccol);
    l1->SetLineWidth(2); l2->SetLineWidth(2); l3->SetLineWidth(2); l4->SetLineWidth(2);
    l1->Draw("same"); l2->Draw("same"); l3->Draw("same"); l4->Draw("same");

    TText *ct = new TText(cnd.sPeak, uHi + 0.035*(uRangeMax-uRangeMin), Form("%d", cnd.peakIndex));
    ct->SetTextColor(ccol);
    ct->SetTextSize(0.027);
    ct->SetTextAlign(22);
    ct->Draw("same");
  }

  for (auto &p : peaks) {
    TLine *lp = new TLine(p.s, uRangeMin, p.s, uRangeMax);
  ...
  TLine *u0 = new TLine(sMin, 0.0, sMax, 0.0);
  u0->SetLineColor(kBlue+1);
  u0->SetLineStyle(2);
  u0->SetLineWidth(2);
  u0->Draw("same");

  tx.DrawLatex(0.10, 0.94, "Transformed coordinate diagnostic: density in (s,u); red=s peaks, green/orange=candidate rectangles, blue=u=0");
  tx.DrawLatex(0.10, 0.90, Form("s runs along the local band; u is transverse. best peak=%d Ys=%d purity=%.3f n=%d", best.peakIndex, best.yscol, best.purity, best.nSelected));
  c->Print(outPdf);
  gPad->SetLogz(0);

  // Page 2: same coordinate system drawn back in the physical YpFp/YFp plane.
  c->Clear();
  gPad->SetLogz(1);
  hYAll->Draw("colz");
  DrawAxisCoordinateGrid(basis, peaks, candidates, sMin, sMax, uMinDraw, uMaxDraw);
  tx.DrawLatex(0.10, 0.94, "Principal-axis coordinate diagnostic in YpFp/YFp: red=s peaks, black dashed=s boundaries, blue=u lines");
  ...
  tx.DrawLatex(0.12, 0.94, "1D density along principal-axis coordinate s; red=peaks, black=valley/capped boundaries");
  c->Print(outPdf);
  ...
  cout << "  find more peaks: .x test_yfp_ypfp_axisvalley_coordinate_debug2.C(" << nrun << ",\"" << outTag << "_morePeaks\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << ",0.035,0.30,0," << uSigmaScale << "," << minUHalf << "," << maxUHalf << "," << minPurity << "," << maxNeighborFrac << ")" << endl;
  cout << "  narrower transverse guard: .x test_yfp_ypfp_axisvalley_coordinate_debug2.C(" << nrun << ",\"" << outTag << "_narrowU\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << "," << minPeakSepS << "," << smoothPassesS << ",1.25," << minUHalf << "," << maxUHalf << "," << minPurity << "," << maxNeighborFrac << ")" << endl;
