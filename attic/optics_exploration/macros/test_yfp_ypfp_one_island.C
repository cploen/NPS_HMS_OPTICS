...
// Usage:
//   hcana -l
//   .x test_yfp_ypfp_one_island.C(1544,"oneIsland_test1","-1","multifoil_test1",0,2)
//
...
TCutG* MakeCandidateCut(const char *name, const LocalCandidate &cand) {
  ...
  auto setp = [&](int i, double s, double u) {
    double x = cand.mx + s * cand.vx + u * cand.ux;
    double y = cand.my + s * cand.vy + u * cand.uy;
    cut->SetPoint(i, x, y);
  };
...
void test_yfp_ypfp_one_island(Int_t nrun=1544,
                              ...
                              Double_t padScaleS=0.40,
                              Double_t padScaleU=0.55)
{
  gStyle->SetOptStat(0);
  ...
  for (size_t ip = 0; ip < peaks.size(); ip++) {
    LocalCandidate cand = FloodFromPeak(hSmooth, peaks[ip].first, peaks[ip].second,
                                        (int)ip, localRelLevel, absFloodThreshold,
                                        maxFloodBins);
    if ((int)cand.bins.size() < minFloodBins) continue;
    CharacterizeCandidate(cand, padScaleS, padScaleU, 0.0012, 0.9);
    cand.cut = MakeCandidateCut(Form("candidate_%zu", ip), cand);
  ...
  TString outPdf = Form("plots/test_yfp_ypfp_one_island_run%d_%s.pdf", nrun, outTag.Data());

  TCanvas *c = new TCanvas("c_test_one_island", "test one YpFp/YFp island", 1100, 850);
  TLatex tx;
  tx.SetNDC();
  ...
  cout << "  tighter core: .x test_yfp_ypfp_one_island.C(" << nrun << ",\"" << outTag << "_tight\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << ",0.65," << padScaleS << "," << padScaleU << ")" << endl;
  cout << "  looser core:  .x test_yfp_ypfp_one_island.C(" << nrun << ",\"" << outTag << "_loose\",\"" << inputID << "\",\"" << ytarTag << "\"," << foilIndex << "," << ndel << "," << minPeakFracOfMax << ",0.45," << padScaleS << "," << padScaleU << ")" << endl;
