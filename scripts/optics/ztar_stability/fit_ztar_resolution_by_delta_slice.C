...
void fit_ztar_resolution_by_delta_slice(
  ...
  TString outPDF = Form("plots/ztar_stability/ztar_delta_slice_resolution_%s", label.Data());
  TString outPDF_individual = Form("plots/ztar_stability/ztar_delta_slice_resolution_by_run_%s", label.Data());
  TString outTSV = Form("ztar_delta_slice_resolution_%s.tsv", label.Data());

  gSystem->mkdir("plots/ztar_stability", kTRUE);
