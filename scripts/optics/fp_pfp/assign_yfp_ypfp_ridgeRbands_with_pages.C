// assign_yfp_ypfp_ridgeRbands_with_pages.C
// Diagnostic: use pooled-local PCA to estimate shared island slant, then separate islands using 1D peaks in r = along the ridge-center coordinate after a second PCA rotation.
//
...
//   hcana -l -q 'assign_yfp_ypfp_ridgeRbands_with_pages.C(1544,-5,0,"auto_ycut")'
//
// Useful tuning, if needed:
//   hcana -l -q 'assign_yfp_ypfp_ridgeRbands_with_pages.C(1544,-5,0,"auto_ycut",7,0.55,0.45)' 
...
void assign_yfp_ypfp_ridgeRbands_with_pages(Int_t nrun=1544,
  ...
  // --- Second rotation: find ridge/chain direction in the already-rotated (u,v) space. ---
  // The first transform describes individual island shape. This second transform describes
  // the direction along which island centers line up. The final band coordinate is r,
  // along that ridge direction. This is intentional: in low-delta slices the island
  // centers separate along the ridge, while w measures only the narrow residual across it.
  double mu=0.0, mv=0.0;
  for (Long64_t i=0; i<N; ++i) { mu += uvals[i]; mv += vvals[i]; }
  mu /= (double)N;
  mv /= (double)N;

  double cuu=0.0, cvv=0.0, cuv=0.0;
  for (Long64_t i=0; i<N; ++i) {
    const double du = uvals[i] - mu;
    const double dv = vvals[i] - mv;
    cuu += du*du;
    cvv += dv*dv;
    cuv += du*dv;
  }
  ...
  double ridgeLam1, ridgeLam2, cr, sr, thetaRidge;
  Eigen2x2Symmetric_vbands(cuu, cvv, cuv, ridgeLam1, ridgeLam2, cr, sr, thetaRidge);
  // Choose a visually stable sign convention.
  if (cr < 0) { cr = -cr; sr = -sr; }
  const double thetaRidgeDegUV = std::atan2(sr, cr) * 180.0 / TMath::Pi();

  std::vector<double> rvals, wvals;
  rvals.reserve(N); wvals.reserve(N);
  for (Long64_t i=0; i<N; ++i) {
    const double du = uvals[i] - mu;
    const double dv = vvals[i] - mv;
    const double r =  du*cr + dv*sr;
    const double w = -du*sr + dv*cr;
    rvals.push_back(r);
    wvals.push_back(w);
  }
  ...
  double wmin=*std::min_element(wvals.begin(), wvals.end());
  double wmax=*std::max_element(wvals.begin(), wvals.end());
  double rpad=0.10*(rmax-rmin), wpad=0.10*(wmax-wmin);
  rmin-=rpad; rmax+=rpad; wmin-=wpad; wmax+=wpad;

  TH2D* hRidge = new TH2D("hRidge", Form("Ridge-PCA coords after pooled-local PCA, Run %d;r = along center ridge;w = across center ridge", nrun),
                          220, rmin, rmax, 220, rmin, rmax);
  TH1D* hR = new TH1D("hR", Form("Run %d;r = along ridge;counts", nrun), 220, rmin, rmax);
  TH1D* hW = new TH1D("hW", Form("Run %d;w = across ridge;counts", nrun), 220, rmin, rmax);

  for (Long64_t i=0; i<N; ++i) {
    hRidge->Fill(rvals[i], wvals[i]);
    hR->Fill(rvals[i]);
    hW->Fill(wvals[i]);
  }

  cout << "\n=== Ridge PCA in pooled-local (u,v) coordinates ===" << endl;
  cout << "Mean u=" << mu << ", mean v=" << mv << endl;
  cout << "Cov(u,v) = [" << cuu << ", " << cuv << "; " << cuv << ", " << cvv << "]" << endl;
  cout << "lambda1/lambda2 ridge: " << ridgeLam1 << " / " << ridgeLam2 << endl;
  cout << "Ridge PC1 angle in (u,v): " << thetaRidgeDegUV << " deg" << endl;
  cout << "Ridge PC1 vector in (u,v): (" << cr << ", " << sr << ")" << endl;

  // --- Find 1D peaks in r, then assign ridge-bands by midpoint boundaries. ---
  // r is the coordinate along the center ridge. In the low-delta case, this is the
  // coordinate that separates the islands; w is mostly the residual width across the ridge.
  TH1D* hRSmooth = (TH1D*)hR->Clone("hRSmooth");
  hRSmooth->Smooth(2);

  struct RPeak { double r=0.0; double height=0.0; int bin=0; };
  std::vector<RPeak> rcands;
  const double rmaxContent = hRSmooth->GetMaximum();
  const double minHeightR = minPeakFraction * rmaxContent;
  for (int ib=2; ib<hRSmooth->GetNbinsX(); ++ib) {
    const double ym = hRSmooth->GetBinContent(ib-1);
    const double y0 = hRSmooth->GetBinContent(ib);
    const double yp = hRSmooth->GetBinContent(ib+1);
    if (y0 > ym && y0 >= yp && y0 > minHeightR) {
      rcands.push_back({hRSmooth->GetBinCenter(ib), y0, ib});
    }
  }
  std::sort(rcands.begin(), rcands.end(), [](const RPeak& a, const RPeak& b){ return a.height > b.height; });

  std::vector<RPeak> rpeaks;
  int nRejectedTooClose = 0;
  for (const auto& cand : rcands) {
    bool tooClose=false;
    for (const auto& pk : rpeaks) {
      if (std::fabs(cand.r - pk.r) < minVPeakSep) { tooClose=true; break; }
    }
    if (tooClose) { nRejectedTooClose++; continue; }
    rpeaks.push_back(cand);
    if ((int)rpeaks.size() >= maxVBands) break;
  }
  std::sort(rpeaks.begin(), rpeaks.end(), [](const RPeak& a, const RPeak& b){ return a.r < b.r; });

  std::vector<double> rbounds;
  for (size_t i=0; i+1<rpeaks.size(); ++i) rbounds.push_back(0.5*(rpeaks[i].r + rpeaks[i+1].r));

  std::vector<int> bandIndex(N, -1);
  std::vector<int> bandCounts(rpeaks.size(), 0);
  for (Long64_t i=0; i<N; ++i) {
    int b=0;
    while (b < (int)rbounds.size() && rvals[i] > rbounds[b]) b++;
    if (b >= 0 && b < (int)rpeaks.size()) { bandIndex[i]=b; bandCounts[b]++; }
  }

  cout << "\n=== r-band peak assignment ===" << endl;
  cout << "Max allowed r peaks: " << maxVBands << endl;
  cout << "Minimum r-peak separation: " << minVPeakSep << endl;
  cout << "Minimum r-peak height fraction: " << minPeakFraction << endl;
  cout << "Candidate r peaks above height threshold: " << rcands.size() << endl;
  cout << "Rejected candidate r peaks as too close to stronger accepted peak: " << nRejectedTooClose << endl;
  cout << "Accepted r peaks: " << rpeaks.size() << endl;
  for (size_t i=0; i<rpeaks.size(); ++i) {
    cout << "  rband " << i << ": peak r=" << rpeaks[i].r
         << ", smoothed height=" << rpeaks[i].height
         << ", assigned events=" << bandCounts[i] << endl;
  }
  cout << "Boundaries:";
  for (double b : rbounds) cout << " " << b;
  cout << endl;

  TString outbase = Form("plots/yfp_ypfp_ridgeRband_pages_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
  ...
  std::vector<TGraph*> graphs(rpeaks.size(), nullptr);
  for (size_t b=0; b<rpeaks.size(); ++b) graphs[b] = new TGraph();
  for (Long64_t i=0; i<N; ++i) {
    int b = bandIndex[i];
  ...
  auto drawConstRLine = [&](double r0, int color, int style, int width) {
    TLine* line = new TLine(mu + r0*cr - wmin*sr, mv + r0*sr + wmin*cr,
                            mu + r0*cr - wmax*sr, mv + r0*sr + wmax*cr);
  ...
  TCanvas* c1 = new TCanvas("c_ridgeband_assign", "Pooled local PC1 + ridge-band assignment", 1200, 950);
  c1->Divide(2,2);
  ...
  lat.DrawLatex(0.12,0.92,Form("Local PC1 %.2f deg z; ridge %.2f deg in uv; r peaks=%zu", thetaDegZ, thetaRidgeDegUV, rpeaks.size()));

  c1->cd(2);
  gPad->SetLogz();
  hRot->Draw("COLZ");
  // Ridge direction through the cloud center in (u,v).
  TLine* ridgeLine = new TLine(mu + rmin*cr, mv + rmin*sr, mu + rmax*cr, mv + rmax*sr);
  ridgeLine->SetLineColor(kOrange+7);
  ridgeLine->SetLineWidth(4);
  ridgeLine->Draw("same");
  for (const auto& pk : rpeaks) drawConstRLine(pk.r, kRed+1, 1, 2);
  for (double b : rbounds) drawConstRLine(b, kRed+1, 2, 2);

  c1->cd(3);
  hR->Draw("HIST");
  hRSmooth->SetLineColor(kRed+1);
  hRSmooth->SetLineWidth(2);
  hRSmooth->Draw("HIST SAME");
  for (const auto& pk : rpeaks) {
    TLine* lpk = new TLine(pk.r, 0, pk.r, hR->GetMaximum()*1.05);
    lpk->SetLineColor(kRed+1);
    lpk->SetLineWidth(2);
    lpk->Draw("same");
  }
  for (double b : rbounds) {
    TLine* lb = new TLine(b, 0, b, hR->GetMaximum()*1.05);
  ...
  c1->cd(4);
  TH2D* hFrame = new TH2D("hFrameRidgeBands", Form("ridge/r-band assignments, Run %d;ypfp;yfp", nrun), 10, xmin, xmax, 10, ymin, ymax);
  ...
  std::vector<double> bandLow(rpeaks.size(), rmin), bandHigh(rpeaks.size(), rmax);
  for (size_t b=0; b<rpeaks.size(); ++b) {
    bandLow[b]  = (b==0) ? rmin : rbounds[b-1];
    bandHigh[b] = (b+1==rpeaks.size()) ? rmax : rbounds[b];
  }

  c1->SaveAs(outpdf + "[");
  c1->SaveAs(outpdf);

  std::vector<TH2D*> hBandOrig(rpeaks.size(), nullptr);
  std::vector<TH2D*> hBandRot(rpeaks.size(), nullptr);
  std::vector<TH2D*> hBandRidge(rpeaks.size(), nullptr);
  std::vector<TH1D*> hBandR(rpeaks.size(), nullptr);
  ...
  for (size_t b=0; b<rpeaks.size(); ++b) {
    ...
    hBandRot[b] = new TH2D(Form("hBandRot_%zu", b), Form("Band %zu only in pooled-local coords, Run %d;u;v", b, nrun), 220, umin, umax, 220, vmin, vmax);
    hBandRidge[b] = new TH2D(Form("hBandRidge_%zu", b), Form("Band %zu only in ridge coords, Run %d;r;w", b, nrun), 220, rmin, rmax, 220, rmin, rmax);
    hBandR[b] = new TH1D(Form("hBandR_%zu", b), Form("Band %zu r distribution, Run %d;r;counts", b, nrun), 220, rmin, rmax);
  }

  for (Long64_t i=0; i<N; ++i) {
    int b = bandIndex[i];
    if (b < 0 || b >= (int)rpeaks.size()) continue;
    hBandOrig[b]->Fill(xvals[i], yvals[i]);
    hBandRot[b]->Fill(uvals[i], vvals[i]);
    hBandRidge[b]->Fill(rvals[i], wvals[i]);
    hBandR[b]->Fill(rvals[i]);
  }

  for (size_t b=0; b<rpeaks.size(); ++b) {
    TCanvas* cb = new TCanvas(Form("c_ridge_r_band_%zu", b), Form("Ridge-r band %zu diagnostics", b), 1200, 950);
    cb->Divide(2,2);
    ...
    pc1->Draw("same"); pc2->Draw("same");
    lat.DrawLatex(0.12,0.92,Form("Ridge-r band %zu highlighted in original coords", b));
    lat.DrawLatex(0.12,0.88,Form("peak r=%.3f, range [%.3f, %.3f], N=%d", rpeaks[b].r, bandLow[b], bandHigh[b], bandCounts[b]));
    ...
    galluv->SetMarkerStyle(20); galluv->SetMarkerSize(0.10); galluv->SetMarkerColor(kGray+1); galluv->Draw("P SAME");
    gseluv->SetMarkerStyle(20); gseluv->SetMarkerSize(0.22); gseluv->SetMarkerColor(colors[b % colors.size()]); gseluv->Draw("P SAME");
    ridgeLine->Draw("same");
    for (const auto& pk : rpeaks) drawConstRLine(pk.r, kRed+1, 2, 1);
    drawConstRLine(rpeaks[b].r, colors[b % colors.size()], 1, 3);
    drawConstRLine(bandLow[b], colors[b % colors.size()], 2, 3);
    drawConstRLine(bandHigh[b], colors[b % colors.size()], 2, 3);

    cb->cd(3);
    hR->Draw("HIST");
    hRSmooth->SetLineColor(kRed+1); hRSmooth->SetLineWidth(2); hRSmooth->Draw("HIST SAME");
    double ymaxW = hR->GetMaximum()*1.08;
    for (const auto& pk : rpeaks) {
      TLine* lpk = new TLine(pk.r, 0, pk.r, ymaxW);
      lpk->SetLineColor(kRed+1); lpk->SetLineWidth(pk.r == rpeaks[b].r ? 3 : 1); lpk->SetLineStyle(pk.r == rpeaks[b].r ? 1 : 2); lpk->Draw("same");
    }
    TLine* bw1 = new TLine(bandLow[b], 0, bandLow[b], ymaxW);
    TLine* bw2 = new TLine(bandHigh[b], 0, bandHigh[b], ymaxW);
    bw1->SetLineColor(colors[b % colors.size()]); bw2->SetLineColor(colors[b % colors.size()]);
    bw1->SetLineWidth(3); bw2->SetLineWidth(3); bw1->SetLineStyle(2); bw2->SetLineStyle(2);
    bw1->Draw("same"); bw2->Draw("same");
    hBandR[b]->SetLineColor(colors[b % colors.size()]); hBandR[b]->SetLineWidth(2); hBandR[b]->Draw("HIST SAME");
    ...
    lat2.DrawLatex(0.12,0.92,Form("Ridge-r band %zu only", b));
    lat2.DrawLatex(0.12,0.87,Form("N=%d events", bandCounts[b]));
    lat2.DrawLatex(0.12,0.82,Form("r peak=%.3f", rpeaks[b].r));
  ...
  hW->Write();
  hRSmooth->Write();
  for (size_t b=0; b<graphs.size(); ++b) graphs[b]->Write(Form("g_ridgeband_%zu", b));
  for (size_t b=0; b<rpeaks.size(); ++b) {
    hBandOrig[b]->Write();
    hBandRot[b]->Write();
    hBandRidge[b]->Write();
  ...
  fout.WriteObject(c1, "c_ridgeband_assign_overview");
  fout.Close();
