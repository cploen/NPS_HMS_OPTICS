// estimate_yfp_ypfp_pc1.C
// Diagnostic: estimate global PC1 / shared long-axis direction in the HMS ypfp-yfp event cloud.
// Default test: run 1544, delta in [-5,0] %, single foil at ztar ~= 0.
//
// Run from repo top directory, e.g.
//   hcana -l -q 'estimate_yfp_ypfp_pc1.C(1544,-5,0,"auto_ycut")'
// or
//   root -l -q 'estimate_yfp_ypfp_pc1.C(1544,-5,0,"auto_ycut")'
...
void estimate_yfp_ypfp_pc1(Int_t nrun=1544,
  ...
  // 2D symmetric eigensystem by closed form.
  ...
  // PC1 angle relative to x=ypfp axis.
  const double theta = 0.5 * std::atan2(2.0*sxy, sxx - syy);
  const double c = std::cos(theta);
  const double s = std::sin(theta);
  const double thetaDeg = theta * 180.0 / TMath::Pi();

  cout << "Mean x=<ypfp>: " << mx << endl;
  cout << "Mean y=<yfp>:  " << my << endl;
  cout << "Cov matrix in plot coords [x=ypfp,y=yfp]:" << endl;
  cout << "  [ " << sxx << "   " << sxy << " ]" << endl;
  cout << "  [ " << sxy << "   " << syy << " ]" << endl;
  cout << "lambda1/lambda2: " << lam1 << " / " << lam2 << endl;
  cout << "PC1 angle from +ypfp axis: " << thetaDeg << " deg" << endl;
  cout << "PC1 unit vector: (" << c << ", " << s << ") in (ypfp,yfp)" << endl;
  ...
  TH2D* hRot = new TH2D("hRot", Form("Rotated coords, Run %d;u = PC1 coordinate;v = PC2 coordinate", nrun),
                        220, umin, umax, 220, vmin, vmax);
  TH1D* hU = new TH1D("hU", Form("Run %d;u = along PC1;counts", nrun), 220, umin, umax);
  TH1D* hV = new TH1D("hV", Form("Run %d;v = across PC1;counts", nrun), 220, vmin, vmax);
  ...
  TString outbase = Form("plots/yfp_ypfp_pc1_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
  ...
  TLine* pc1 = new TLine(mx - L*c, my - L*s, mx + L*c, my + L*s);
  pc1->SetLineColor(kOrange+7);
  pc1->SetLineWidth(4);
  pc1->Draw("same");
  TLine* pc2 = new TLine(mx + L*s, my - L*c, mx - L*s, my + L*c);
  ...
  lat.DrawLatex(0.14,0.92,Form("PC1 angle = %.2f deg from +ypfp", thetaDeg));
