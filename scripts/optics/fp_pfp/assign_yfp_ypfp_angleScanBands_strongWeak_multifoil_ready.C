...
//   hcana -l -q 'assign_yfp_ypfp_angleScanBands_strongWeak_multifoil_ready.C(1544,-10,-8,"ytar_ridge_cut")'
//
// Useful test:
//   hcana -l -q 'assign_yfp_ypfp_angleScanBands_strongWeak_multifoil_ready.C(1544,-8,-5,"ytar_ridge_cut",9,1.0,0.18,0.05,0.15,2,0.005,0.04,0.45)'
...
static TCutG* LoadYtarCut_angleScan(int nrun, const TString& ytarTag, int foilIndex=0) {
  // Clean final naming convention first:
  //   cuts/<ytarTag>_<run>.root
  // Example:
  //   cuts/ytar_ridge_cut_1544.root
  // Fallbacks are kept only so older dev artifacts can still be read during transition.
  std::vector<TString> candidateFiles = {
    Form("cuts/%s_%d.root", ytarTag.Data(), nrun),
    Form("cuts/ytar_delta_%d_%s_multifoil_cut.root", nrun, ytarTag.Data()),
    Form("cuts/ytar_delta_%d_%s_multifoil_noexpert_multifoil_cut.root", nrun, ytarTag.Data()),
    Form("cuts/ytar_delta_%d_multifoil_noexpert_multifoil_cut.root", nrun)
  };

  TFile* f = nullptr;
  TString fname = "";
  for (const auto& cand : candidateFiles) {
    f = TFile::Open(cand, "READ");
    if (f && !f->IsZombie()) {
      fname = cand;
      break;
    }
  ...
  if (!f || f->IsZombie()) {
    cout << "WARNING: could not open any ytar cut file for tag=" << ytarTag
         << ", run=" << nrun << endl;
    cout << "Tried:" << endl;
    for (const auto& cand : candidateFiles) cout << "  " << cand << endl;
    return nullptr;
  }
  ...
  std::vector<TString> names = {
    ...
    Form("ytar_ridge_cut_foil%d", foilIndex),
...
void assign_yfp_ypfp_angleScanBands_strongWeak_multifoil_ready(Int_t nrun=1544,
  ...
  cout << "Foil index: " << foilIndex << endl;
  cout << "Ytar tag: " << ytarTag << endl;
  ...
  TString safeTag = ytarTag;
  safeTag.ReplaceAll("/", "_");
  safeTag.ReplaceAll(" ", "_");

  TString outbase = Form("plots/yfp_ypfp_angleScanStrongWeak_run%d_foil%d_delta_%g_to_%g_%s",
                         nrun, foilIndex, deltaMin, deltaMax, safeTag.Data());
