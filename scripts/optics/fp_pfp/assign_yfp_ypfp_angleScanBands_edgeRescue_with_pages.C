// assign_yfp_ypfp_angleScanBands_edgeRescue_with_pages.C
//
...
//   hcana -l -q 'assign_yfp_ypfp_angleScanBands_edgeRescue_with_pages.C(1544,-10,-8,"auto_ycut")'
//
// Useful test:
//   hcana -l -q 'assign_yfp_ypfp_angleScanBands_edgeRescue_with_pages.C(1544,-8,-5,"auto_ycut",9,1.0,0.18,0.05,0.15,0.005,0.04,2)'
...
static std::vector<QPeak> FindPeaks1D_angleScan(TH1D* hSmooth,
                                                 ...
                                                 double minRescuePeakFraction,
                                                 double minRescueProminenceFraction) {
  struct CandPeak {
    double q = 0.0;
    double height = 0.0;
    double prominence = 0.0;
  ...
  const double primaryMinHeight = minPeakFraction * maxContent;
  const double rescueMinHeight  = minRescuePeakFraction * maxContent;

  // Collect all local maxima above the lower rescue threshold.
  for (int ib=2; ib<hSmooth->GetNbinsX(); ++ib) {
    ...
    if (y0 > ym && y0 >= yp && y0 > rescueMinHeight) {
  ...
  std::vector<QPeak> primaryCands;
  std::vector<QPeak> rescueCands;
  ...
  for (size_t i=0; i<localMaxima.size(); ++i) {
    ...
    QPeak pk{localMaxima[i].q, h0, prom, localMaxima[i].bin};

    if (h0 >= primaryMinHeight && prom >= minProminenceFraction) {
      primaryCands.push_back(pk);
    } else if (h0 >= rescueMinHeight && prom >= minRescueProminenceFraction) {
      rescueCands.push_back(pk);
    }
  }

  // Keep strongest/prominent primary peaks first.
  std::sort(primaryCands.begin(), primaryCands.end(),
  ...
  std::vector<QPeak> peaks;
  for (const auto& cand : primaryCands) {
    bool tooClose=false;
  ...
  // Rescue weak but real peaks from the tails or large gaps.
  // This specifically handles low-stat outer islands that are visible in q but
  // get swallowed by the neighboring dominant peak.
  std::sort(peaks.begin(), peaks.end(),
            [](const QPeak& a, const QPeak& b){ return a.q < b.q; });

  double medianSpacing = minPeakSep;
  if (peaks.size() >= 2) {
    std::vector<double> spacings;
    for (size_t i=0; i+1<peaks.size(); ++i) spacings.push_back(peaks[i+1].q - peaks[i].q);
    std::sort(spacings.begin(), spacings.end());
    medianSpacing = spacings[spacings.size()/2];
  }
  const double largeGap = std::max(1.75*medianSpacing, 1.25*minPeakSep);

  std::sort(rescueCands.begin(), rescueCands.end(),
            [](const QPeak& a, const QPeak& b){
              if (a.prominence != b.prominence) return a.prominence > b.prominence;
              return a.height > b.height;
            });

  for (const auto& cand : rescueCands) {
    if ((int)peaks.size() >= maxPeaks) break;
    ...
    bool rescueThis = false;

    if (peaks.empty()) rescueThis = true;
    else if (cand.q < peaks.front().q - minPeakSep) rescueThis = true;  // left tail
    else if (cand.q > peaks.back().q + minPeakSep) rescueThis = true;   // right tail
    else {
      for (size_t i=0; i+1<peaks.size(); ++i) {
        if (cand.q > peaks[i].q + minPeakSep &&
            cand.q < peaks[i+1].q - minPeakSep &&
            (peaks[i+1].q - peaks[i].q) > largeGap) {
          rescueThis = true;
          break;
        }
      }
    ...
    if (rescueThis) peaks.push_back(cand);
  }
...
static AngleResult ScoreAngle_angleScan(const std::vector<double>& xzvals,
  ...
  res.thetaRad = th;
  res.peaks = FindPeaks1D_angleScan(hS, maxBands, minPeakSep, minPeakFraction, minProminenceFraction, minRescuePeakFraction, minRescueProminenceFraction);
...
void assign_yfp_ypfp_angleScanBands_edgeRescue_with_pages(Int_t nrun=1544,
                                                ...
                                                Double_t minRescuePeakFraction=0.005,
                                                Double_t minRescueProminenceFraction=0.04,
  ...
  cout << "Rescue peak height fraction: " << minRescuePeakFraction << endl;
  cout << "Rescue peak prominence fraction: " << minRescueProminenceFraction << endl;
  ...
  for (double th=0.0; th<180.0; th += thetaStepDeg) {
    scan.push_back(ScoreAngle_angleScan(xzvals, yzvals, th,
                                         ...
                                         minRescuePeakFraction, minRescueProminenceFraction,
                                         smoothPasses));
  }
  ...
  TString outbase = Form("plots/yfp_ypfp_angleScanEdgeBand_pages_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
