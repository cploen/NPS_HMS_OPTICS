// assign_yfp_ypfp_ridgebands_with_pages.C
...
//   hcana -l -q 'assign_yfp_ypfp_ridgebands_with_pages.C(1544,-5,0,"auto_ycut")'
//
// Useful tuning, if needed:
//   hcana -l -q 'assign_yfp_ypfp_ridgebands_with_pages.C(1544,-5,0,"auto_ycut",7,0.55,0.45)' 
...
void assign_yfp_ypfp_ridgebands_with_pages(Int_t nrun=1544,
  ...
  // the direction along which island centers line up. The final band coordinate is w,
  // perpendicular to that ridge direction.
  ...
  TH2D* hRidge = new TH2D("hRidge", Form("Ridge-PCA coords after pooled-local PCA, Run %d;r = along center ridge;w = across center ridge", nrun),
                          220, rmin, rmax, 220, wmin, wmax);
  TH1D* hR = new TH1D("hR", Form("Run %d;r = along ridge;counts", nrun), 220, rmin, rmax);
  TH1D* hW = new TH1D("hW", Form("Run %d;w = across ridge;counts", nrun), 220, wmin, wmax);
  ...
  // --- Find 1D peaks in w, then assign ridge-bands by midpoint boundaries. ---
  TH1D* hWSmooth = (TH1D*)hW->Clone("hWSmooth");
  hWSmooth->Smooth(2);

  struct WPeak { double w=0.0; double height=0.0; int bin=0; };
  std::vector<WPeak> wcands;
  const double wmaxContent = hWSmooth->GetMaximum();
  const double minHeightW = minPeakFraction * wmaxContent;
  for (int ib=2; ib<hWSmooth->GetNbinsX(); ++ib) {
    const double ym = hWSmooth->GetBinContent(ib-1);
    const double y0 = hWSmooth->GetBinContent(ib);
    const double yp = hWSmooth->GetBinContent(ib+1);
    if (y0 > ym && y0 >= yp && y0 > minHeightW) {
      wcands.push_back({hWSmooth->GetBinCenter(ib), y0, ib});
    }
  }
  std::sort(wcands.begin(), wcands.end(), [](const WPeak& a, const WPeak& b){ return a.height > b.height; });

  std::vector<WPeak> wpeaks;
  int nRejectedTooClose = 0;
  for (const auto& cand : wcands) {
    bool tooClose=false;
    for (const auto& pk : wpeaks) {
      if (std::fabs(cand.w - pk.w) < minVPeakSep) { tooClose=true; break; }
    }
    if (tooClose) { nRejectedTooClose++; continue; }
    wpeaks.push_back(cand);
    if ((int)wpeaks.size() >= maxVBands) break;
  }
  std::sort(wpeaks.begin(), wpeaks.end(), [](const WPeak& a, const WPeak& b){ return a.w < b.w; });

  std::vector<double> wbounds;
  for (size_t i=0; i+1<wpeaks.size(); ++i) wbounds.push_back(0.5*(wpeaks[i].w + wpeaks[i+1].w));

  std::vector<int> bandIndex(N, -1);
  std::vector<int> bandCounts(wpeaks.size(), 0);
  for (Long64_t i=0; i<N; ++i) {
    int b=0;
    while (b < (int)wbounds.size() && wvals[i] > wbounds[b]) b++;
    if (b >= 0 && b < (int)wpeaks.size()) { bandIndex[i]=b; bandCounts[b]++; }
  }

  cout << "\n=== w-band peak assignment ===" << endl;
  cout << "Max allowed w peaks: " << maxVBands << endl;
  cout << "Minimum w-peak separation: " << minVPeakSep << endl;
  cout << "Minimum w-peak height fraction: " << minPeakFraction << endl;
  cout << "Candidate w peaks above height threshold: " << wcands.size() << endl;
  cout << "Rejected candidate w peaks as too close to stronger accepted peak: " << nRejectedTooClose << endl;
  cout << "Accepted w peaks: " << wpeaks.size() << endl;
  for (size_t i=0; i<wpeaks.size(); ++i) {
    cout << "  wband " << i << ": peak w=" << wpeaks[i].w
         << ", smoothed height=" << wpeaks[i].height
         << ", assigned events=" << bandCounts[i] << endl;
  }
  cout << "Boundaries:";
  for (double b : wbounds) cout << " " << b;
  cout << endl;

  TString outbase = Form("plots/yfp_ypfp_ridgeband_pages_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
  ...
  std::vector<TGraph*> graphs(wpeaks.size(), nullptr);
  for (size_t b=0; b<wpeaks.size(); ++b) graphs[b] = new TGraph();
  for (Long64_t i=0; i<N; ++i) {
    int b = bandIndex[i];
  ...
  auto drawConstWLine = [&](double w0, int color, int style, int width) {
    TLine* line = new TLine(mu + rmin*cr - w0*sr, mv + rmin*sr + w0*cr,
                            mu + rmax*cr - w0*sr, mv + rmax*sr + w0*cr);
  ...
  lat.DrawLatex(0.12,0.92,Form("Local PC1 %.2f deg z; ridge %.2f deg in uv; w peaks=%zu", thetaDegZ, thetaRidgeDegUV, wpeaks.size()));
  ...
  ridgeLine->Draw("same");
  for (const auto& pk : wpeaks) drawConstWLine(pk.w, kRed+1, 1, 2);
  for (double b : wbounds) drawConstWLine(b, kRed+1, 2, 2);

  c1->cd(3);
  hW->Draw("HIST");
  hWSmooth->SetLineColor(kRed+1);
  hWSmooth->SetLineWidth(2);
  hWSmooth->Draw("HIST SAME");
  for (const auto& pk : wpeaks) {
    TLine* lpk = new TLine(pk.w, 0, pk.w, hW->GetMaximum()*1.05);
    lpk->SetLineColor(kRed+1);
    lpk->SetLineWidth(2);
    lpk->Draw("same");
  }
  for (double b : wbounds) {
    TLine* lb = new TLine(b, 0, b, hW->GetMaximum()*1.05);
  ...
  c1->cd(4);
  TH2D* hFrame = new TH2D("hFrameRidgeBands", Form("ridge/w-band assignments, Run %d;ypfp;yfp", nrun), 10, xmin, xmax, 10, ymin, ymax);
  ...
  std::vector<double> bandLow(wpeaks.size(), wmin), bandHigh(wpeaks.size(), wmax);
  for (size_t b=0; b<wpeaks.size(); ++b) {
    bandLow[b]  = (b==0) ? wmin : wbounds[b-1];
    bandHigh[b] = (b+1==wpeaks.size()) ? wmax : wbounds[b];
  }

  c1->SaveAs(outpdf + "[");
  c1->SaveAs(outpdf);

  std::vector<TH2D*> hBandOrig(wpeaks.size(), nullptr);
  std::vector<TH2D*> hBandRot(wpeaks.size(), nullptr);
  std::vector<TH2D*> hBandRidge(wpeaks.size(), nullptr);
  std::vector<TH1D*> hBandW(wpeaks.size(), nullptr);
  ...
  for (size_t b=0; b<wpeaks.size(); ++b) {
    ...
    hBandRidge[b] = new TH2D(Form("hBandRidge_%zu", b), Form("Band %zu only in ridge coords, Run %d;r;w", b, nrun), 220, rmin, rmax, 220, wmin, wmax);
    hBandW[b] = new TH1D(Form("hBandW_%zu", b), Form("Band %zu w distribution, Run %d;w;counts", b, nrun), 220, wmin, wmax);
  }

  for (Long64_t i=0; i<N; ++i) {
    int b = bandIndex[i];
    if (b < 0 || b >= (int)wpeaks.size()) continue;
    hBandOrig[b]->Fill(xvals[i], yvals[i]);
    hBandRot[b]->Fill(uvals[i], vvals[i]);
    hBandRidge[b]->Fill(rvals[i], wvals[i]);
    hBandW[b]->Fill(wvals[i]);
  }

  for (size_t b=0; b<wpeaks.size(); ++b) {
    TCanvas* cb = new TCanvas(Form("c_ridge_band_%zu", b), Form("Ridge band %zu diagnostics", b), 1200, 950);
    cb->Divide(2,2);
    ...
    lat.DrawLatex(0.12,0.92,Form("Ridge band %zu highlighted in original coords", b));
    lat.DrawLatex(0.12,0.88,Form("peak w=%.3f, range [%.3f, %.3f], N=%d", wpeaks[b].w, bandLow[b], bandHigh[b], bandCounts[b]));
    ...
    ridgeLine->Draw("same");
    for (const auto& pk : wpeaks) drawConstWLine(pk.w, kRed+1, 2, 1);
    drawConstWLine(wpeaks[b].w, colors[b % colors.size()], 1, 3);
    drawConstWLine(bandLow[b], colors[b % colors.size()], 2, 3);
    drawConstWLine(bandHigh[b], colors[b % colors.size()], 2, 3);

    cb->cd(3);
    hW->Draw("HIST");
    hWSmooth->SetLineColor(kRed+1); hWSmooth->SetLineWidth(2); hWSmooth->Draw("HIST SAME");
    double ymaxW = hW->GetMaximum()*1.08;
    for (const auto& pk : wpeaks) {
      TLine* lpk = new TLine(pk.w, 0, pk.w, ymaxW);
      lpk->SetLineColor(kRed+1); lpk->SetLineWidth(pk.w == wpeaks[b].w ? 3 : 1); lpk->SetLineStyle(pk.w == wpeaks[b].w ? 1 : 2); lpk->Draw("same");
    }
    ...
    hBandW[b]->SetLineColor(colors[b % colors.size()]); hBandW[b]->SetLineWidth(2); hBandW[b]->Draw("HIST SAME");
    ...
    lat2.DrawLatex(0.12,0.92,Form("Ridge band %zu only", b));
    lat2.DrawLatex(0.12,0.87,Form("N=%d events", bandCounts[b]));
    lat2.DrawLatex(0.12,0.82,Form("w peak=%.3f", wpeaks[b].w));
    lat2.DrawLatex(0.12,0.77,Form("w range [%.3f, %.3f]", bandLow[b], bandHigh[b]));
