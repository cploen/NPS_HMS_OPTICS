...
static TCutG* LoadYtarCut_angleScan(int nrun, const TString& ytarTag, int foilIndex=0, int numFoil=1) {
  TString fnames = Form("cuts/ytar_ridge_cut__run%d.root",  nrun);
  TFile* f = TFile::Open(fnames, "READ");
  if (!f || f->IsZombie()) {
...
void assign_yfp_ypfp_angleScanBands_strongWeak_with_pages_foilIndex(Int_t nrun=1544,
  ...
  TString outbase = Form("plots/yfp_ypfp_angleScanStrongWeakBand_pages_run%d_foil%d_delta_%g_to_%g", nrun, foilIndex, deltaMin, deltaMax);
