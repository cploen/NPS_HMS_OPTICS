// assign_xfp_xpfp_angleScanBands_watershed_with_pages.C
//
...
//   hcana -l -q 'assign_xfp_xpfp_angleScanBands_watershed_with_pages.C(1544,-10,-8,"auto_ycut")'
//
// Useful test:
//   hcana -l -q 'assign_xfp_xpfp_angleScanBands_watershed_with_pages.C(1544,-8,-5,"auto_ycut",9,1.0,0.18,0.05,0.15,2,0.005,0.04,0.45)'
...
void assign_xfp_xpfp_angleScanBands_watershed_with_pages(Int_t nrun=1544,
                                                ...
                                                Double_t shoulderMinPeakFraction=0.003,
                                                Double_t shoulderMinProminenceFraction=0.015,
                                                Double_t shoulderMinSepFactor=0.35,
                                                Bool_t enableOuterShoulderSplit=true,
  ...
  cout << "Outer shoulder/watershed split enabled: " << enableOuterShoulderSplit << endl;
  cout << "Shoulder peak fraction: " << shoulderMinPeakFraction << endl;
  cout << "Shoulder prominence fraction: " << shoulderMinProminenceFraction << endl;
  cout << "Shoulder min separation factor: " << shoulderMinSepFactor << endl;
  ...
  // -------------------------------------------------------------------------
  // Optional 1D weak watershed / shoulder split.
  // This is deliberately AFTER angle choice and AFTER strong+weak peak finding.
  // Purpose: split a low-stat outer shoulder from its neighboring dominant peak
  // when the q-histogram shows a distinct blob but strict local-max logic merged it.
  // By default this only tests the two outer diagnostic bands to avoid over-splitting
  // legitimate strongly connected interior x-plane ridges.
  // bandIsWeak: 0=strong, 1=weak local max, 2=weak shoulder/watershed split.
  // -------------------------------------------------------------------------
  int nShoulderAdded = 0;
  if (enableOuterShoulderSplit && bandPeaks.size() > 0) {
    std::vector<double> tmpBounds = BoundariesFromPeaks_angleScan(bandPeaks);

    auto bandLowTmp = [&](size_t ib)->double {
      return (ib==0) ? qmin : tmpBounds[ib-1];
    };
    auto bandHighTmp = [&](size_t ib)->double {
      return (ib+1==bandPeaks.size()) ? qmax : tmpBounds[ib];
    };

    auto tryAddShoulder = [&](size_t ib, bool leftSide) {
      if ((int)bandPeaks.size() >= maxBands) return;

      const double q0 = bandPeaks[ib].q;
      const int centerBin = hQSmooth->FindBin(q0);
      const double globalMax = hQSmooth->GetMaximum();
      const double minShoulderHeight = shoulderMinPeakFraction * globalMax;
      const double minSep = shoulderMinSepFactor * minPeakSep;

      int loBin = hQSmooth->FindBin(bandLowTmp(ib));
      int hiBin = hQSmooth->FindBin(bandHighTmp(ib));

      int searchLo = loBin;
      int searchHi = hiBin;
      if (leftSide) {
        searchHi = hQSmooth->FindBin(q0 - minSep);
      } else {
        searchLo = hQSmooth->FindBin(q0 + minSep);
      }

      searchLo = std::max(1, searchLo);
      searchHi = std::min(hQSmooth->GetNbinsX(), searchHi);
      if (searchHi <= searchLo + 2) return;

      int bestBin = -1;
      double bestH = -1.0;
      for (int b=searchLo; b<=searchHi; ++b) {
        const double h = hQSmooth->GetBinContent(b);
        if (h > bestH) { bestH = h; bestBin = b; }
      }
      if (bestBin < 0 || bestH < minShoulderHeight) return;

      const double qCand = hQSmooth->GetBinCenter(bestBin);
      if (std::fabs(qCand - q0) < minSep) return;

      const double valley = LocalValleyMin_angleScan(hQSmooth, bestBin, centerBin);
      const double prom = (bestH > 0.0) ? (bestH - valley)/bestH : 0.0;
      if (prom < shoulderMinProminenceFraction) return;

      // Avoid duplicating an existing center.
      for (const auto& pk : bandPeaks) {
        if (std::fabs(qCand - pk.q) < minSep) return;
      }

      QPeak shoulder;
      shoulder.q = qCand;
      shoulder.height = bestH;
      shoulder.prominence = prom;
      shoulder.bin = bestBin;

      bandPeaks.push_back(shoulder);
      bandIsWeak.push_back(2);
      nShoulderAdded++;
    };

    // Outer bands only: left side of first band and right side of last band.
    // This targets the common failure mode without touching the clean interior ridges.
    tryAddShoulder(0, true);
    if (bandPeaks.size() > 1) tryAddShoulder(bandPeaks.size()-1, false);

    // Re-sort after any shoulder additions.
    std::vector<size_t> order2(bandPeaks.size());
    for (size_t i=0; i<order2.size(); ++i) order2[i]=i;
    std::sort(order2.begin(), order2.end(),
              [&](size_t a, size_t b){ return bandPeaks[a].q < bandPeaks[b].q; });

    std::vector<QPeak> sorted2;
    std::vector<int> weak2;
    for (size_t idx : order2) {
      sorted2.push_back(bandPeaks[idx]);
      weak2.push_back(bandIsWeak[idx]);
    }
    bandPeaks.swap(sorted2);
    bandIsWeak.swap(weak2);
  }

  std::vector<double> bandBounds = BoundariesFromPeaks_angleScan(bandPeaks);

  auto bandLabel = [&](int flag)->const char* {
    if (flag == 0) return "strong";
    if (flag == 1) return "weak";
    if (flag == 2) return "shoulder";
    return "unknown";
  };

  cout << "\n=== Final strong+weak+shoulder q-band centers ===" << endl;
  ...
  cout << "Outer shoulder/watershed peaks added: " << nShoulderAdded << endl;
  cout << "Final diagnostic bands: " << bandPeaks.size() << endl;
  for (size_t i=0; i<bandPeaks.size(); ++i) {
    cout << "  qband " << i
         << " [" << bandLabel(bandIsWeak[i]) << "]"
  ...
  TString outbase = Form("plots/xfp_xpfp_angleScanWatershedBand_pages_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
  ...
  lat.DrawLatex(0.12,0.92,Form("Best #theta = %.2f deg z-space; bands=%zu, weak=%d, shoulder=%d", best.thetaDeg, bandPeaks.size(), nWeakAdded, nShoulderAdded));
  ...
  for (size_t b=0; b<bandPeaks.size(); ++b) {
    ...
    lat.DrawLatex(0.12,0.92,Form("Angle band %zu [%s] highlighted in original coords", b, bandLabel(bandIsWeak[b])));
    ...
    lat2.DrawLatex(0.12,0.92,Form("Angle band %zu [%s] only", b, bandLabel(bandIsWeak[b])));
