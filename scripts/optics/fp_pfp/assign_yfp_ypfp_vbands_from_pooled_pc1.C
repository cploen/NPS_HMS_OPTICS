// assign_yfp_ypfp_vbands_from_pooled_pc1.C
...
//   hcana -l -q 'assign_yfp_ypfp_vbands_from_pooled_pc1.C(1544,-5,0,"auto_ycut")'
//
// Useful tuning, if needed:
//   hcana -l -q 'assign_yfp_ypfp_vbands_from_pooled_pc1.C(1544,-5,0,"auto_ycut",7,0.55,0.45)'
// where the last numbers are: nVPeaksTarget, peakSepZ, coreRadiusZ, minVPeakSep.
...
void assign_yfp_ypfp_vbands_from_pooled_pc1(Int_t nrun=1544,
                                               Double_t deltaMin=-5.0,
                                               Double_t deltaMax=0.0,
                                               TString ytarTag="auto_ycut",
                                               Int_t nVPeaksTarget=7,
  ...
  cout << "Requested local PCA density peaks: " << nVPeaksTarget << endl;
  ...
  for (const auto& cand : candidates) {
    ...
    if ((int)peaks.size() >= nVPeaksTarget) break;
  }

  if ((int)peaks.size() < nVPeaksTarget) {
  ...
  const double minHeight = 0.05 * vmaxContent;
  ...
  for (const auto& cand : vcands) {
    ...
    if ((int)vpeaks.size() >= nVPeaksTarget) break;
  }
  ...
  cout << "Requested v peaks: " << nVPeaksTarget << endl;
  cout << "Minimum v-peak separation: " << minVPeakSep << endl;
  cout << "Found v peaks: " << vpeaks.size() << endl;
  ...
  TString outbase = Form("plots/yfp_ypfp_vband_assign_run%d_delta_%g_to_%g", nrun, deltaMin, deltaMax);
  ...
  fout.WriteObject(c1, "c_vband_assign");
  fout.Close();
