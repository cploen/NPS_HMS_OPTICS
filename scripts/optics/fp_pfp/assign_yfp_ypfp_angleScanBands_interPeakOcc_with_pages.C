...
// winning command for 1540 +8 cm foil: hcana -l -q 'assign_yfp_ypfp_angleScanBands_interPeakOcc_with_pages.C(1540,-5,0,"auto_ycut",9,1.0,0.18,0.05,0.30,2,0.005,0.04,0.45,1,1,-1,"test1540_f1_interOcc_prom030")'
//
...
struct AngleResult {
  ...
  double thetaRad = 0.0;
  int nPeaks = 0;
  double score = 0.0;              // robust score used for choosing best theta
  double oldScore = 0.0;           // original score, kept for comparison
  double totalPeakHeight = 0.0;
  double meanProminence = 0.0;
  double meanValleyDrop = 0.0;
  double meanInterPeakOccupancy = 0.0; // avg counts between neighboring peaks / lower peak height
...
static double MeanInterPeakOccupancy_angleScan(TH1D* hSmooth, const std::vector<QPeak>& peaks) {
  // For each adjacent peak pair, average the smoothed histogram content
  // between the two peak bins, then normalize by the lower neighboring peak.
  // Low value = empty valley / clean separation.
  // High value = filled valley / overlapping or fake peaks.
  ...
  for (size_t i=0; i+1<peaks.size(); ++i) {
    ...
    // Exclude the peak bins themselves. If peaks are adjacent, fall back to
    // using the direct interval including endpoints so the metric remains finite.
    int firstBin = b1 + 1;
    int lastBin  = b2 - 1;
    if (firstBin > lastBin) {
      firstBin = b1;
    ...
    for (int b=firstBin; b<=lastBin; ++b) {
      sumBetween += hSmooth->GetBinContent(b);
      nBetween++;
    }

    if (nBetween > 0) {
      const double avgBetween = sumBetween / (double)nBetween;
      sumOcc += avgBetween / lowPeak;
      nPairs++;
    }
  }

  return (nPairs > 0) ? sumOcc / (double)nPairs : 1.0;
}
...
static AngleResult ScoreAngle_angleScan(const std::vector<double>& xzvals,
                                         ...
                                         double minProminenceFraction,
                                         int smoothPasses,
                                         double interPeakPenaltyWeight=2000000.0,
                                         int nBinsQ = 240) {
  ...
  res.meanInterPeakOccupancy = MeanInterPeakOccupancy_angleScan(hS, res.peaks);

  double meanProm = 0.0;
  for (const auto& pk : res.peaks) meanProm += pk.prominence;
  if (!res.peaks.empty()) meanProm /= (double)res.peaks.size();
  res.meanProminence = meanProm;

  // Original score retained for diagnosis/comparison.
  res.oldScore = 1000000.0 * res.nPeaks
               + 100000.0 * meanProm
               + 10000.0 * res.meanValleyDrop
               + 0.1 * res.totalPeakHeight;

  // Robust separation score used for choosing best theta.
  // Same core terms as before, but now filled inter-peak regions are expensive.
  // This allows a clean 6-peak solution to beat a fake/overlapping 7-peak solution.
  res.score = res.oldScore - interPeakPenaltyWeight * res.meanInterPeakOccupancy;
...
void assign_yfp_ypfp_angleScanBands_interPeakOcc_with_pages(Int_t nrun=1544,
                                                ...
                                                TString outTag="interPeakOcc",
                                                Double_t interPeakPenaltyWeight=2000000.0) {
  ...
  cout << "Output tag: " << outTag << endl;
  cout << "Inter-peak occupancy penalty weight: " << interPeakPenaltyWeight << endl;
  ...
  for (double th=0.0; th<180.0; th += thetaStepDeg) {
    scan.push_back(ScoreAngle_angleScan(xzvals, yzvals, th,
                                         ...
                                         smoothPasses, interPeakPenaltyWeight));
  }
  ...
  cout << "Robust score: " << best.score << endl;
  cout << "Original score: " << best.oldScore << endl;
  cout << "Mean prominence: " << best.meanProminence << endl;
  cout << "Mean valley drop: " << best.meanValleyDrop << endl;
  cout << "Mean inter-peak occupancy: " << best.meanInterPeakOccupancy << endl;
  ...
  TString safeTag = outTag;
  safeTag.ReplaceAll("/", "_");
  safeTag.ReplaceAll(" ", "_");
  TString outbase = Form("plots/yfp_ypfp_angleScanInterPeakOcc_pages_%s_run%d_foil%d_delta_%g_to_%g", safeTag.Data(), nrun, foilIndex, deltaMin, deltaMax);
  ...
  lat.DrawLatex(0.12,0.88,Form("inter-peak occ=%.3f, valley=%.3f", best.meanInterPeakOccupancy, best.meanValleyDrop));
  ...
  TGraph* gOldScore = new TGraph();
  TGraph* gNPeaks = new TGraph();
  TGraph* gValley = new TGraph();
  TGraph* gOcc = new TGraph();
  for (size_t i=0; i<scan.size(); ++i) {
    gScore->SetPoint(i, scan[i].thetaDeg, scan[i].score);
    gOldScore->SetPoint(i, scan[i].thetaDeg, scan[i].oldScore);
    gNPeaks->SetPoint(i, scan[i].thetaDeg, scan[i].nPeaks);
    gValley->SetPoint(i, scan[i].thetaDeg, scan[i].meanValleyDrop);
    gOcc->SetPoint(i, scan[i].thetaDeg, scan[i].meanInterPeakOccupancy);
  }

  c1->SaveAs(outpdf + "[");
  c1->SaveAs(outpdf);

  TCanvas* cScore = new TCanvas("c_angle_scan_score", "Angle scan score", 1200, 1400);
  cScore->Divide(1,5);

  cScore->cd(1);
  gScore->SetTitle(Form("Robust angle score with inter-peak occupancy penalty, Run %d, %.1f<#delta<%.1f;theta (deg);robust score", nrun, deltaMin, deltaMax));
  ...
  cScore->cd(2);
  gOldScore->SetTitle("Original score retained for comparison;theta (deg);original score");
  gOldScore->SetLineWidth(2);
  gOldScore->Draw("AL");
  TLine* bestOldScoreLine = new TLine(best.thetaDeg, gPad->GetUymin(), best.thetaDeg, gPad->GetUymax());
  bestOldScoreLine->SetLineColor(kRed+1); bestOldScoreLine->SetLineWidth(2); bestOldScoreLine->Draw("same");
  ...
  cScore->cd(5);
  gOcc->SetTitle("Mean inter-peak occupancy vs angle;theta (deg);avg counts between peaks / lower peak height");
  gOcc->SetLineWidth(2);
  gOcc->Draw("AL");
  TLine* bestOccLine = new TLine(best.thetaDeg, gPad->GetUymin(), best.thetaDeg, gPad->GetUymax());
  bestOccLine->SetLineColor(kRed+1); bestOccLine->SetLineWidth(2); bestOccLine->Draw("same");
  ...
  hP->Write();
  hQSmooth->Write();
  gScore->Write("g_angle_robust_score");
  gOldScore->Write("g_angle_original_score");
  gNPeaks->Write("g_angle_npeaks");
  gValley->Write("g_angle_valley");
  gOcc->Write("g_angle_inter_peak_occupancy");
