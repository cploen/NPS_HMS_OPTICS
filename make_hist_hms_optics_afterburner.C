...
void make_hist_hms_optics_afterburner(Int_t nrun=1544,Bool_t CutYtarFlag=kTRUE,Bool_t CutYpFpYFpFlag=kTRUE,Bool_t CutXpFpXFpFlag=kTRUE,Int_t FileID=-2){
 ...
 //
  ...
  if (CutYtarFlag) {
   ...
   for (Int_t nc=0;nc<NumFoil;nc++) {
      fYtarDeltaCut->cd();
      TCutG* tempcut = (TCutG*)fYtarDeltaCut->Get(Form("delta_vs_ytar_cut_foil%d",nc));
      if (tempcut) {
...
//
  ...
  if (CutXpFpXFpFlag) {
	...
	for  (Int_t nd=0;nd<ndelcut;nd++) {
        for (Int_t nc=0;nc<9;nc++) {
	  TCutG* tempg  = (TCutG*)xpfp_xfp_fcut->Get(Form("hXpFpXFp_cut_yscol_%d_nfoil_%d_ndel_%d",nc,nf,nd));
	  if (tempg)  {
